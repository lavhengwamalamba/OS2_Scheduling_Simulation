...
public class SchedulingSimulation {
	...
	public static void main(String[] args) throws InterruptedException, IOException {
		...
		writer = new FileWriter("metric_times_" + Integer.toString(sched) +"_with_"+ noPatrons + "_patrons.csv", false);
		...
		for (int i = 0; i < noPatrons; i++) {
			patrons[i] = new Patron(i, startSignal, Andre, sched, count); // we are adding schedular to the constructor
																			// so that the patron knows how its done
			patrons[i].start();
		}
